...
/**
 * Values from our prefs/options for Digger.
 */
...
/**
 * Values from our prefs/options for Logicker.
 */
class LogickerConfiguration {
    ...
    static KNOWN_BAD_IMG_REGEX = new RegExp(
        '\\/(logo\\.|loading|header\\.jpg|premium_|preview\\.png|holder-trailer-home\\.jpg|logo-mobile-w\\.svg|logo\\.svg|logo-desktop-w\\.svg|user\\.svg|speech\\.svg|folder\\.svg|layers\\.svg|tag\\.svg|video\\.svg|favorites\\.svg|spinner\\.svg|preview\\.jpg)',
        'i'
    );
    static MESSAGES = [];
    static PROCESSINGS = [];
...
/**
 * Values from our prefs/options for Output.
 */
class OutputConfiguration {
    static ENABLE_HALF_BAKED = false;
}


/**
 * Values from our prefs/options for Utils.
 */
class UtilsConfiguration {
    static LISTENER_TIMED_OUT = 'Listener timed out';
    static LISTENER_TIMEOUT_MS = 7000;
    static NUM_DL_CONCURRENT = 10;
    static DEFAULT_IFRAME = 'background_iframe';
    static DL_SPACING_MS = 675;
}
...
class OptionsConfiguration {
    ...
    static LABELS = {
        CONFIG: {
            ...
            concurrentDls: 'number of file-results downloaded concurrently',
            dlChannels: 'number of download channels for batches of gallery-gallery-digs',
    ...
    // These are only the default spec values. They get returned if the key isn't found in localstorage.
...
class DominatrixConfiguration {
    ...
    static SCOPE_DIV_D_SUBENTRY_SEL = `${SelectorProperties.SCOPE} ${SelectorProperties.DIV}.${DominatrixConfiguration.SUB_ENTRY_CLASS}`;
    static SCOPE_BUTTON_D_DELETE_BUTTON_SEL = `${SelectorProperties.SCOPE} ${SelectorProperties.BUTTON}.${DominatrixConfiguration.DELETE_BUTTON_CLASS}`;
 }
...
class RecognizingRegex {
    static ALL_MEDIA = /(\'|\")?.+?\.(jpg|jpeg|png|gif|tiff|mpg|mpeg|mp4|flv|avi|zip|tar|gz|mp3|ogg|aac|m4a)([\?].+?)?(\'|\")?/g;
    ...
    static SUPPORTED = /\.(jpg|jpeg|png|gif|tiff|mpg|mpeg|mp4|flv|avi|zip|tar|gz|mp3|ogg|aac|m4a)(\?.+?)?$/i;
}
...
class MimeTypeRegex {
    ...
    static ALLSUPPORTED = /(image|application|audio|video|text)\/(jpg|jpeg|png|gif|tiff|mpg|mpeg|mp4|flv|avi|zip|tar|gz|mp3|ogg|aac|m4a|html|xhtml|rtf|pdf|octet-stream)/;
}
...
class BlankValue {
    ...
    static OBJ = {};
    static ARR = [];
    static MATCH_ALL_REGEX = '.+';
    static MATCH_NONE_REGEX = '!(.+)';
    static DEF_INT = 10;
}
...
class CannedFunctions {
    static E = ( () => undefined );
    static PR_RJ = ( (outVal) => Promise.reject((!!outVal ? outVal : BlankValue.PR_RJ)) ); 
    static PR_RS = ( (outVal) => Promise.resolve((!!outVal ? outVal : BlankValue.PR_RJ)) );
...
class C {
    ...
    static LGK_CONF = LogickerConfiguration;
    static OUT_CONF = OutputConfiguration;
    static UTILS_CONF = UtilsConfiguration;
    static OPT_CONF = OptionsConfiguration;
    static DX_CONF = DominatrixConfiguration;

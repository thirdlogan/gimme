...
class CommonStaticBase {
    ...
    static setup(logSrc) {
        ...
        this.listenerKey = C.KEY.LISTENER_PREFIX + (CommonStaticBase.counter++) + C.ST.E;
        this.stop = false;
        
        // Listen for the stop event. It should be a once-in-an-app-instance, so remove
        // the event listener when we get the stop event.
        CommonStaticBase.stopListeners[this.listenerKey] = (evt) => {
        ...
        window.document.addEventListener(C.ACTION.STOP, CommonStaticBase.stopListeners[this.listenerKey]);

        // Listen for the resume event. It might not exist yet, but will, to keep a 
        // dig going after stop has canceled a batch or two.
        CommonStaticBase.resumeListeners[this.listenerKey] = (evt) => {
        ...
        window.document.addEventListener(C.ACTION.RESUME, CommonStaticBase.resumeListeners[this.listenerKey]);
    }
